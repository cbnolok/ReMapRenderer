...
void CalcNormal(float *Vertex1, float *Vertex2, float *Vertex3, float *result)
{
  float V1[3];
  float V2[3];
...
float CheckSphereNearPlane(float *sphere, float tx, float ty)
{
  ...
  for (p = 0; p < 6; p++) {
    d =
      frustum[p][0] * (sphere[0] + tx) + frustum[p][1] * (sphere[1] + ty) +
      frustum[p][2] * sphere[2] + frustum[p][3] + sphere[3];
    if((d <= 0.0f) && (p < 5))
      return 0.0f;
  }
...
float GetSquaredDistRayPoint(float vecOrigin[3], float vecDir[3], float vecPoint[3], float & lambda)
{
...
bool IntersectPlaneWithRay(float vecOrigin[], float vecDir[], float vecP[], float vecNormal[], float result[], float & lambda)
{
	int i;
...
bool IntersectTriangleWithRay(float vecOrigin[], float vecDir[], float vecP1[], float vecP2[], float vecP3[], float & lambda)
{
	...
	if (!IntersectPlaneWithRay(vecOrigin, vecDir, vecP1, vecNormal, vecX,  lambda))
		return false;
...
bool IntersectTriangleWithRay2(float vecOrigin[], float vecDir[], float vecP1[], float vecP2[], float vecP3[], float & lambda)
{
	...
	float detD[9];
	float DMain, Dalpha, Dbeta, Dtest;
	
	detD[0] = vecP1[0]; detD[3] = vecP1[1]; detD[6] = vecP1[2];
	detD[1] = vecU[0]; detD[4] = vecU[1]; detD[7] = vecU[2];
	detD[2] = vecV[0]; detD[5] = vecV[1]; detD[8] = vecV[2];
	DMain = CalcDeterminant(detD);
	
	detD[0] = vecP1[0]; detD[3] = vecP1[1]; detD[6] = vecP1[2];
	detD[1] = vecU[0]; detD[4] = vecU[1]; detD[7] = vecU[2];
	detD[2] = vecX[0]; detD[5] = vecX[1]; detD[8] = vecX[2];
